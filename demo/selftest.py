...
check(
    "epoch local naive -> UTC en verano (CEST, -2h)",
    t == pd.Timestamp("2026-08-15T19:08:52Z"),
    f"obtenido {t}",
)
...
check(
    "mismo campo en invierno (CET, -1h)",
    t_inv == pd.Timestamp("2026-01-15T08:00:00Z"),
    f"obtenido {t_inv}",
)
check(
    "ISO naive de Renfe -> UTC",
    local_naive_iso_to_utc("2026-08-15T21:11:56")
    == pd.Timestamp("2026-08-15T19:11:56Z"),
)
...
d = parse("emt_buses", pl, ing)
check(
    "dato rancio -> se marca DUDOSA en vez de adivinar",
    d["tz_convencion"].iloc[0] == "DUDOSA",
    d["tz_convencion"].iloc[0],
)
...
check(
    "serie con convenciones alternas: ninguna fila desplazada >1 h",
...
check(
    "trafico_intensidad: lectura=-1 se convierte en nulo",
    df_int["lectura"].isna().sum() == 1,
    f"nulos={df_int['lectura'].isna().sum()}",
)

df_emt = parse("emt_buses", FIX["emt_buses"], ts)
check(
    "emt: snapshot_id = gid mínimo del bloque",
...
sim = (
    sim.sample(frac=1, random_state=3).sort_values("snapshot_id").reset_index(drop=True)
)
...
print(
    f"     ingenuo    : identidad correcta {acc_ing:.1%}  | {r_ing['trayectorias']} trayectorias  "
    f"| vel p50 {r_ing['vel_kmh_p50']} km/h"
)
print(
    f"     predictivo : identidad correcta {acc_pred:.1%}  | {r_pred['trayectorias']} trayectorias  "
    f"| vel p50 {r_pred['vel_kmh_p50']} km/h"
)

check(
    "tracker: 60 buses simulados -> 60 trayectorias reconstruidas",
    r_pred["trayectorias"] == 60,
    f"obtenidas {r_pred['trayectorias']}",
)
check(
    "tracker predictivo: identidad correcta >= 99 %",
    acc_pred >= 0.99,
    f"{acc_pred:.1%}",
)
check(
    "tracker predictivo mejora al ingenuo",
    acc_pred >= acc_ing,
    f"{acc_pred:.1%} vs {acc_ing:.1%}",
)
check(
    "tracker: emparejamiento por encima del 95 %",
    r_pred["tasa_emparejamiento"] >= 0.95,
    f"tasa={r_pred['tasa_emparejamiento']}",
)
check(
    "tracker: velocidades reconstruidas dentro del rango simulado (3-30 km/h)",
    2.0 <= r_pred["vel_kmh_p50"] <= 32.0,
    f"p50={r_pred['vel_kmh_p50']}",
)
...
check(
    "gids de dos sondeos consecutivos son disjuntos",
    len(b0 & b1) == 0,
