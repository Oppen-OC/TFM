...
def construir(root: Path, con_senal: bool, semilla: int) -> None:
    ...
    for k in range(n_bus):
        ...
        for ln in range(N_LINEAS):
            ...
            filas_b.append(
                {
                    "snapshot_id": 1_000_000 + k,
                    "gid": 1_000_000 + k * N_LINEAS + ln,
                    "linea": f"L{ln}",
                    "trayecto": f"L{ln}-Ida",
                    "lat": lat + rng.normal(0, 8e-6),
                    "lon": lon + rng.normal(0, 8e-6),
                    "ts_utc": ts,
                    "ts_ingest_utc": ts,
                    "latencia_s": 30.0,
                }
