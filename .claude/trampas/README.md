...
|----|--------|------|--------|---------|
| [001](001-gid-no-identifica-vehiculo.md) | cerrada | fuentes | `gid` identifica al refresco, no al vehículo: la tabla se trunca y reinserta entera, intersección cero entre sondeos. Solo vale de `snapshot_id`; la identidad se infiere | `test_gids_…_disjuntos` |
...
| [008](008-puerta-fisica-sobre-posicion-predicha.md) | cerrada | tracking | La puerta `min(SALTO_MAX_M, VEL_MAX_KMH·dt)` se evaluaba sobre la posición **predicha**, no sobre el desplazamiento real: colaban buses a 110 km/h sin error alguno. Coste y restricción no pueden compartir matriz | `test_ningun_desplazamiento_supera_la_puerta_fisica` |
